...
def _at_z(records,z,bounds):
    ...
    p=np.asarray([r['points_uz'] for r in records]);dz=p[:,1,1]-p[:,0,1]
...
def apply_internal_competitions(branches,route,graph,target_s,*,policy=None):
    ...
    for region in sorted(regions.values(),key=lambda r:(r['bounds'][4],r['region_id'])):
        ...
        for z in sorted(positions,key=lambda z:(abs(z-midpoint),z)):
            current=[x for x in _runs(records) if _at_z(records[x[0]:x[1]],z,bounds)]
            if len(current)!=1:
            ...
            for bid in ids:
                ...
                if bid not in future:
                    future[bid]=future_switch_cost(reliable,bid,z_direction=1,
                        target_z=max(b['z_range'][1] for b in reliable),policy=policy,reliable_entry=True)['estimated_additional_switches']
                rows.append(dict(m,**face,in_ASC=any(m['ASC_start_arc']<=h['arc_position']<=m['ASC_end_arc'] for h in hits),
                    forward_Z_m=max(0.,by[bid]['z_range'][1]-z),
                    forward_ASC_m=max(max(0.,m['ASC_end_arc']-max(h['arc_position'],m['ASC_start_arc'])) for h in hits),
                    minimum_switches=future[bid]))
            if len(rows)<2 or not any(r['branch_id']==aid for r in rows):continue
            item['evaluated_positions']+=1;pending=rows;rank=0
            while pending:
                decision=choose_successor(pending,current_branch_id=aid);winner=decision['selected'];rank+=1
                ...
                for piece in pieces:
                    entry,ed=reliable_junction(terminal,piece,a,by[bid],metrics[aid],metrics[bid],
                        ...
                        preserve_extent=False,junction_z_bounds=(-np.inf,entry_z))
                    ...
                    for suffix in remaining_a:
                        ...
                        exitj,xd=reliable_junction(b_retained,suffix,by[bid],a,metrics[bid],metrics[aid],
                            required=route['route_z_extent'],locked_range=(np.inf,-np.inf),direction='upper',policy=policy,
                            preserve_extent=False,junction_z_bounds=(exit_z,np.inf))
                        ...
                        budgets=route_budgets(candidate,by,metrics,policy)
                        ...
                        for j,source,target in [(entry,aid,bid),(exitj,bid,aid)]:
                            ...
                            j.update(connector_gate(j['xyz_distance_m'],observed,policy),
                                ...
                                identity_rank=rank,identity_ambiguous=decision['ambiguous'],
    ...
    output=dict(route,**route_result(records,branch_switch_count=len(sequence)-1,junction=joins[0]),
        ...
        local_competition_audit=audit,route_contribution_budgets=route_budgets(records,by,metrics,policy),
